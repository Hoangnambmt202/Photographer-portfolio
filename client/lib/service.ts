  ...
  if (response.data) {
    ...
    return {
      ...
      includedItems: service.included_items
        ? service.included_items.split(",").map((item: string) => item.trim())
        : [],
      rating: 5,
      totalBookings: 0,
    };
...
export async function updateService(id: number, payload: ServiceFormData) {
  ...
  if (payload.price !== undefined)
    apiData.price = parseInt(payload.price.replace(/\D/g, "")) || 0;
  if (payload.duration !== undefined) apiData.duration = payload.duration;
  if (payload.max_people !== undefined)
    apiData.max_people = parseInt(payload.max_people) || null;
  if (payload.included_items !== undefined)
    apiData.included_items = payload.included_items;
  if (payload.status !== undefined) apiData.status = payload.status;
  if (payload.category_id !== undefined)
    apiData.category_id = payload.category_id;
  if (payload.cover_image !== undefined)
    apiData.cover_image = payload.cover_image;
  if (payload.discount_percent !== undefined)
    apiData.discount_percent = payload.discount_percent;
  if (payload.is_featured !== undefined)
    apiData.is_featured = payload.is_featured;
  if (payload.display_order !== undefined)
    apiData.display_order = payload.display_order;
