...
interface SortConfig {
  key: 'id' | 'name' | 'slug'; // Chỉ định các key hợp lệ
  direction: 'asc' | 'desc';
}
...
const sortKeys = {
  ...
  name: 'Tên',
  slug: 'Slug',
  // createdAt: 'Ngày tạo', // Bỏ nếu Tag không có createdAt
};
...
export default function TagTable() {
  ...
  const handleBulkDelete = () => {
    if (confirm(`Bạn có chắc chắn muốn xóa ${selectedTags.length} tag đã chọn?`)) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden">
          ...
          <div className="flex items-center gap-3">
            {selectedTags.length > 0 && (
              <button
                ...
                className="px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white rounded-lg font-medium flex items-center gap-2 transition-colors"
              >
            ...
            <tr>
              ...
              {(Object.keys(sortKeys) as Array<keyof typeof sortKeys>).map((key) => (
                <th
                  ...
                  className="p-4 text-left text-sm font-semibold text-gray-700 uppercase tracking-wider cursor-pointer hover:bg-gray-200 transition-colors group"
                >
      ...
      <DeleteModal
        ...
        item="tag"
        itemName={tags.find((tag) => tag.id === deleteId)?.name}
      />
    </div>
